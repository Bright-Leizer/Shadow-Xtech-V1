...
if (!fs.existsSync(baseDir)) {
    try {
        ...
        console.error('Shadow-Xtech Store Error: Failed to create /app/message_data directory:', e);
    }
}
...
function saveChatData(remoteJid, messageId, chatData) {
    ...
    if (!fs.existsSync(chatDir)) {
        try {
            ...
            console.error('Shadow-Xtech Store Error: Failed to create directory:', e);
        }
    }
    ...
    try {
        ...
        console.error('Shadow-Xtech Store Error: Failed to save chat data:', error);
    }
}
...
function cleanupOldMessages(maxAgeMs = 24 * 60 * 60 * 1000) {
    try {
            ...
            fs.readdirSync(chatDir).forEach(file => {
                ...
                try {
                    ...
                    if (now - stats.mtimeMs > maxAgeMs) {
                        ...
                        // console.log(`Shadow-Xtech Store Cleanup: Deleted old file ${filePath}`);
                    }
                } catch (err) {
                    console.error('Shadow-Xtech Store Cleanup Error:', err);
                }
            });
        ...
        console.error('Shadow-Xtech Store Cleanup Fatal Error:', err);
    }
}
